...
const tracks: Track[] = [
  ...
  {
    id: "02",
    title: <>Artificial Intelligence &amp; AI Systems</>,
  },
...
const topicsMap: Record<number, string[]> = {
  0: [
    ...
    "Quantum Governance",
    "Quantum Talent & Education",
    "Quantum Startups & Ecosystems",
  ],
  1: [
  ...
  2: [
    ...
    "On-chain Gaming",
    "MEV & Fair Ordering",
    "Rollups & Modular Chains",
    "Audits & Security Tooling",
  ],
  3: [
    "Art",
    "Marketing",
    ...
    "Digital & On-Chain Art",
    "Immersive Installations",
    "Narrative Design",
    "Game Design & Play",
    "Myths & Local Lore",
    "Naples Heritage & Culture",
    "Music & Live Performance",
    "Film & Visual Story",
    "Community Rituals & Festivals",
    "Creators & New Patronage",
...
function ThemeAndTracks(): ReactElement {
  ...
  // Build the list of labels (unique topics, shuffled; no repetition)
  const labels = useMemo(() => {
    const baseTopics = topicsMap[activeTrack] ?? [];
    if (!baseTopics.length) return [];

    // shuffle topics and take up to MAX_BUBBLES
    const shuffled = shuffle(baseTopics);
    const takeCount = Math.min(Math.max(MIN_BUBBLES, shuffled.length), MAX_BUBBLES);
  ...
  useEffect(() => {
    ...
    return () => {
      Events.off(engine, "afterUpdate", sync as unknown as (e: Event) => void);
      Runner.stop(runner);
      Engine.clear(engine);
